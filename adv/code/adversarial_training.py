...
if __name__ == "__main__":
    ...
    save_model1(adv_classifier, exp)
    ...
    acc_adv = np.sum(preds == non_encoded_test_labels) / non_encoded_test_labels.shape[0]
    logger.info("Classifier with adversarial training")
    logger.info("Accuracy on adversarial test samples: %.2f%%", (acc * 100))

    print("Evaluate adversarially trained model on clean test samples...\n")
    preds = np.argmax(adv_classifier.predict(test_data), axis=1)
    acc = np.sum(preds == non_encoded_test_labels) / non_encoded_test_labels.shape[0]
    logger.info("Classifier with adversarial training")
    logger.info("Accuracy on clean test samples: %.2f%%", (acc * 100))
