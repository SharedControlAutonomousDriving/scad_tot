...
FOLDERPATH = '/content/drive/MyDrive/SCAD'
exp = 8
...
fileHandler = logging.FileHandler("{0}/{1}{2}.log".format(LOG_DIR, "adv_training",exp))
...
def load_data():
    train = pd.read_csv(FOLDERPATH + '/data/v3.2.2_train.csv')
    train = train.drop(columns=['Unnamed: 0'])
    test = pd.read_csv(FOLDERPATH + '/data/v3.2.2_test.csv')
...
if __name__ == "__main__":
    ...
    logger.info("Attack: BasicIterativeMethod")
    logger.info("="*50)
 
    
    print("Loading model...\n")
    new_model = tf.keras.models.load_model(FOLDERPATH + "/network/models/latest/model.h5")
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.01)
    ...
    bim = BasicIterativeMethod(adv_classifier,eps=eps,eps_step=eps_step,targeted=False,batch_size=2048)
    ...
    adv_classifier.fit(aug_train_data, aug_train_labels, nb_epochs=nepochs, batch_size=2048)
