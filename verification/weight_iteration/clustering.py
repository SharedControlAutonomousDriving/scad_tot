...
class LabelGuidedKMeans:
    ...
    def __init__(self):
     ...
     #Modification made by adding argument wt
    def fit(self, X, Y, init_centroid='mean', weights = None):
        '''
        ...
        while len(remaining) > 0:
            ...
            model = KMeans(**model_params,n_init = 1).fit(X, Y, sample_weight = weights)
...
class LabelGuidedKMeansUtils:
    ...
    def remove_outliers(X, Y, tolerance):
        ...
        return X[idxs], Y[idxs]					#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< added weight
