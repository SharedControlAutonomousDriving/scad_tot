...
def save_local_robustness_results_to_csv(results, asym=False, outdir=default_outdir):
    '''
...
def find_counterexample(net, sample, epsilon, asym_side='', timeout=default_timeout, verbose=0):
    '''
    finds counterexamples for a given epsilon. returns None if no counterexamples found.
    '''
...
def test_local_robustness(nnet_path, samples, e_min=0.00001, e_max=100, e_prec=None, asym=False, save_results=False, save_samples=False, outdir=default_outdir, timeout=default_timeout, verbose=0):
    ...
    assert(e_prec < e_min)
    net = TOTNet(nnet_path, outdir=outdir)
    start = ms_since_1970()
    epsilons = []
    ...
    summary = (-min([le for le,_ in epsilons if le != 0]), min([ue for _,ue in epsilons if ue != 0])) if epsilons else (0, 0)
